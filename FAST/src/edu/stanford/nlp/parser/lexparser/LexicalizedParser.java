...
public class LexicalizedParser implements ViterbiParser, Function<Object,Tree> {
  ...
  public Tree apply(Object in) {
    ...
    try {
      if (parse(lst)) {
        ...
        bestparse.setScore(getPCFGScore() % -1000); // -1000 denotes unknown words
        return bestparse;
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public final ParserData getParserDataFromTreebank(Treebank trainTreebank,
    ...
    Extractor<MLEDependencyGrammar> dgExtractor = op.tlpParams.dependencyGrammarExtractor(op);
  ...
  public double testOnTreebank(Treebank testTreebank) {
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("factTA"))) {
      if (op.doPCFG) {
        ...
        factTA = new AbstractEval.TaggingEval("factor Tag", runningAverages, pd.lex, true);
      }
    }
