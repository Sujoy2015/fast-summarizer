 ...
 * <p/>
 * ToDO: There's still tricky generic stuff to get right here: toArray should
 * take an arg of a different generic type if we follow the Collections API,
 * and the OBIterator doesn't seem to do the generic typing right. Should it
 * rather be F extends E ?  [cdm notes, sep 2007] 
 *
...
public class ObjectBank<E> implements Collection<E>, Serializable {
  ...
  public static <X> ObjectBank<X> getLineIteratorObjectBank(String fileOrString, Function<String,X> op) {
    ...
    c.add(fileOrString);
    return getLineIteratorObjectBank(c, op);
  }
  
  //TODO: Should the "files" collection be required to hold Files?
  public static <X> ObjectBank<X> getLineIteratorObjectBank(Collection files, Function<String,X> op) {
    return getLineIteratorObjectBank(files, op, "utf-8");
  }
  ...
  public static ObjectBank<String> getLineIteratorObjectBank(String filename, String encoding) {
    return getLineIteratorObjectBank(Collections.singleton(filename), new IdentityFunction<String>(), encoding);
  }
  
  public static ObjectBank<String> getLineIteratorObjectBank(String filename) {
    return getLineIteratorObjectBank(filename, "utf-8");
  }
   ...
   * Can be slow.  Usage not recommended.
   */
  //TODO: Not sure if this is right.  It used to have <E>, but that was shadowing the 
  //      class's parametrized type...
